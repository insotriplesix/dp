...
{
	int optval, rc, clnt_len, serv_len, nfds;
	int *_sockfd;
	struct hostent *clnt_host;
	struct sockaddr_in clnt_addr, serv_addr;
	struct epoll_event ev, efds[2];
	...
	while (0x1) {
		nfds = epoll_wait(epollfd, efds, nevents, 0);
		if (nfds < 0) {
			fprintf(stderr, _RED_CLR"[System] "_DEF_CLR);
			perror("epoll");
			exit(EXIT_FAILURE);
		...
		for (int i = 0; i < nfds; ++i) {
			...
			if (efds[i].data.fd == udp_sockfd) {
				...
				if (!is_active_udp_client(ntohs(clnt_addr.sin_port))) {
					printf(_BLUE_CLR"[Server]"_DEF_CLR" established connection with"
